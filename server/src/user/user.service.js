...
export class UserService {
    //Constructor
    constructor(userRepository, sessionRepository, httpService, jwtService) {
    ...
    //Get Users
    async getUsers(searchInput) {
        const users = await this.userRepository
            .createQueryBuilder("user")
            .orderBy("user.created_at", searchInput.orderBy ?? "DESC")
            .where(
            "user.is_verified = :isVerified AND user.role = :role",
        ...
        const { items, meta } = await paginate(users, {
    ...
    //Verify Phone
    async verify(verifyPhoneInput, req) {
    ...
    async adminLogin(loginInput, req) {
        const user = await this.userRepository.findOne({
            where: [
                { phone: loginInput.phoneOrEmail },
                { email: loginInput.phoneOrEmail }
    ...
    async sellerLogin(loginInput, req) {
        const user = await this.userRepository.findOne({
            where: [
                { phone: loginInput.phoneOrEmail },
                { email: loginInput.phoneOrEmail }
    ...
    async facebook({
        ...
        const { data } = await firstValueFrom(this.httpService.get(urlGraphFacebook).pipe(catchError((error) => {
            throw new NotFoundException("Something Went Wrong!")
        })));
        const user = await this.userRepository.findOneBy({
            email: data.id
    ...
    //Update Profile
    async update(updateUserInput, reqUser) {
        const user = await this.userRepository.findOneBy({
            id: reqUser.id
        });
    ...
    //Change Password
    async changePassword(changePasswordInput, reqUser) {
        const user = await this.userRepository.findOne({
            where: {
                id: reqUser.id
            },
    ...
    //Phone Change
    async phoneChange(phoneInput, reqUser) {
        const user = await this.userRepository.findOneBy({
            phone: phoneInput.phone
        });
    ...
    //Phone Change Verify
    async changePhoneVerify(verifyPhoneInput, reqUser) {
