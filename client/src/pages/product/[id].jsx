...
const Product = () => {
    ...
    //Urql
    const [{ data, error }] = useQuery({ query: GET_SINGLE_PRODUCT, variables: { getProductId: id } });

    //State
    const [selected, setSelected] = useState(data?.getProduct.images[0] || "");
...
export const getServerSideProps = async (ctx) => {
    ...
    //ID
    const parts = ctx.query && ctx.query.id?.toString().split("-")
    const id = parts[parts.length - 1];
    ...
    //---//
    await client.query(GET_PROFILE, {}, {
        fetchOptions,
        requestPolicy: "network-only"
    }).toPromise();

    //In Page Queries
    await client.query(GET_SINGLE_PRODUCT, { getProductId: id }, {
        fetchOptions,
        requestPolicy: "network-only"
    }).toPromise();

    //In Page Queries
    await client.query(GET_SELLING_PRODUCT, { getSellingProductId: id }, {
        fetchOptions,
        requestPolicy: "network-only"
    }).toPromise();

    //In Page Queries
    await client.query(CHECK_WISHLIST, { productId: id }, {
        fetchOptions,
        requestPolicy: "network-only"
    }).toPromise();

    //In Page Queries
    await client.query(GET_REVIEWS, { productId: id }, {
        fetchOptions,
        requestPolicy: "network-only"
    }).toPromise();
