...
export const getServerSideProps = async (ctx) => {
  ...
  //In Page Queries
  await client.query(GET_BANNER_ONE, {}, {
    fetchOptions,
    requestPolicy: "network-only"
  }).toPromise();

  //In Page Queries
  await client.query(GET_CATEGORIES, { searchInput: { page: 1, limit: 30 } }, {
    fetchOptions,
    requestPolicy: "network-only"
  }).toPromise();

  //In Page Queries
  await client.query(GET_BANNER_TWO, {}, {
    fetchOptions,
    requestPolicy: "network-only"
  }).toPromise();

  //In Page Queries
  await client.query(GET_BRANDS, { searchInput: { page: 1, limit: 15 } }, {
    fetchOptions,
    requestPolicy: "network-only"
  }).toPromise();

  //In Page Queries
  await client.query(GET_SELLER, { searchInput: { page: 1, limit: 15 } }, {
    fetchOptions,
    requestPolicy: "network-only"
  }).toPromise();

  //In Page Queries
  await client.query(GET_SECTIONS, {}, {
    fetchOptions,
    requestPolicy: "network-only"
  }).toPromise();
