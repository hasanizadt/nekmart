...
const FlashDetails = () => {
    ...
    //Urql
    const [{ data, error }] = useQuery({ query: GET_SINGLE_FLASH, variables: { getFlashId: id } });

    //Timer Setup
    const expiryTimestamp = new Date(data?.getFlash?.expires);
    const {
        seconds,
        minutes,
        hours,
    ...
    return (
        <section>
            <div className="aspect-[25/6] relative">
                {data.getFlash.image &&
                    <Image
                        src={process.env.NEXT_PUBLIC_IMAGE_URL + data.getFlash.image}
                        alt={data.getFlash.title}
                        fill />
                }
            </div>
            <Container>
                <div className="py-8">
                    ...
                    {mount &&
                        <div className="flex gap-4 justify-center mt-5">
                            <p
                                className="w-11 h-10 rounded bg-[#c1c23aff] flex justify-center items-center">
