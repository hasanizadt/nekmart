...
const FacebookLogin = () => {
    ...
    //Urql
    const [{ error, fetching }, mutate] = useMutation(FACEBOOK_LOGIN);

    //Handler
    const onSuccess = (response) => {
        if (response) {
            mutate(
                { facebookInput: { userId: response.userID, accessToken: response.accessToken } }
            ).then(({ error, data }) => {
                if (error) {
            ...
            <LoginButton
                ...
                render={({ onClick }) => (
                    <button
                        onClick={onClick}
                        disabled={fetching}
                        className="w-full bg-[#0062cc] py-2 flex justify-center items-center gap-4 rounded-md text-white relative">
