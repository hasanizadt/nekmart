...
const GoogleLogin = () => {
    return (
        <Fragment>
            <GoogleOAuthProvider
                clientId="342777733000-ceu60md3s942jl51a41pemje6s9jcfav.apps.googleusercontent.com">
                <LoginButton />
            </GoogleOAuthProvider>
        </Fragment>
    );
...
const LoginButton = () => {
    ...
    //Urql
    const [{ error, fetching }, mutate] = useMutation(GOOGLE_LOGIN);
    ...
    return (
        <div>
            ...
            <button
                onClick={() => login()}
                disabled={fetching}
                className="w-full border border-solid border-[#0062cc] py-2 flex justify-center items-center gap-4 rounded-md text-[#0062cc] mt-4 relative">
                <Icon className="googleIcon" icon="flat-color-icons:google" />
                <span>Login with Google</span>
                <div className="absolute top-1/2 -translate-y-1/2 right-5">
                    {fetching &&
                        <div
                            className="w-5 h-5 border-b-2 border-main rounded-full animate-spin ml-auto"></div>
                    }
                </div>
            </button>
        </div>
