...
const VerificationForm = () => {
    ...
    //Urql
    const [{ error: verifyError, fetching: verifyFetching }, verifyMutate] = useMutation(VERIFY_PHONE_NUMBER);
    const [{ data: resendData, error: resendError, fetching: resendFetching }, resendMutate] = useMutation(RESEND_OTP);
    ...
    return (
        <section>
            ...
            <Notification open={verify} handleClose={onVerify} severity="error">
                {verifyError?.message}
            </Notification>
                ...
                <div
                    ...
                    <OtpInput
                        ...
                        renderInput={(props) => <input
                            {...props}
                            className="!w-full !mx-[5px] !text-2xl !px-2 !py-3 rounded-md border border-solid border-black placeholder:text-black placeholder:font-light focus:outline-main"
                            inputMode="text" />} />
                    <div className="mt-8 mb-5">
                        <button
                            className={`bg-main w-full uppercase font-semibold py-3 text-white rounded-md text-sm relative ${otp.length < 6 ? "opacity-60" : ""}`}
                            type="button"
                            disabled={verifyFetching || otp.length < 6}
                    ...
                    <Otptimer
                        ...
                        spinnerComponent={<div
                            className="w-3 h-3 border-b-2 border-main rounded-full animate-spin mt-px"></div>}
