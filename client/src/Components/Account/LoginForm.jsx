...
const LoginForm = () => {
    ...
    //Urql
    const [{ error, fetching }, mutate] = useMutation(PHONE_NUMBER_LOGIN);
    ...
    const onSubmit = (value) => {
        mutate({ phone: "88" + value.phone }).then(({ error, data }) => {
            ...
            if (data?.phoneLogin.success) {
                router.push(
                    `/account/verification?token=${nextBase64.encode(value.phone)}&expires=${moment().add(5, 'minutes').unix()}`
                )
            }
        }).catch(() => {
            setNotification(true)
        ...
        <section>
            <Container className="py-14">
                ...
                <div
                    ...
                    <div className="mt-4">
                        <Link
                            href="/account/old-login"
                            className="bg-main py-2.5 text-center w-full text-white block rounded-md">
                            Login with User ID
                        </Link>
                    </div>
                    <div
                        className="my-4 flex items-center gap-1 whitespace-nowrap after:border-t after:border-gray-300 after:relative after:w-full after:left-0 before:border-t before:border-gray-300 before:relative before:w-full before:left-0">
                        <span>Or</span>
                    </div>
