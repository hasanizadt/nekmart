...
const SellerCard = () => {
    ...
    const [{ data, error }] = useQuery(
        { query: GET_SELLER, variables: { searchInput: { page: page, limit: 20 } } }
    );

    //Error Return
    if (error) {
        return (<p className="text-center text-main font-semibold mt-16">{error.message}</p>);
    }
    ...
    return (
        <section>
            <Container className="py-14">
                ...
                <div className="grid grid-cols-5 gap-6">
                    {data.getSellers.results.map((item, i) => (
                        <div key={i} className="border border-solid border-gray-300 p-5 text-center">
                            ...
                            <Link
                                href={`/shop/${slugify(item.shopName, { lower: true })}-${item.id}`}
                                key={i}
                                className="block w-full bg-main py-2 text-center text-white text-sm rounded-md font-medium mt-2">
                                Visit store
                            </Link>
                        </div>
                    ))}
                ...
                <Pagination
                    totalPages={data?.getSellers?.meta?.totalPages}
                    currentPage={data?.getSellers?.meta?.currentPage}
