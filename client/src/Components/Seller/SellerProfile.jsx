...
const SellerProfile = () => {
    ...
    //Urql
    const [{ data }] = useQuery({ query: GET_SINGLE_SELLER, variables: { getSellerId: id } });
        ...
        <section>
            <Container className="py-12">
                <div className="relative aspect-[16/3]">
                    <Image
                        src={process.env.NEXT_PUBLIC_IMAGE_URL + data.getSeller.banner}
                        alt={data.getSeller.shopName}
                        fill
                        className="rounded-md" />
                    <div className="absolute left-5 top-[78%] flex gap-2 items-end">
                        <Image
                            src={process.env.NEXT_PUBLIC_IMAGE_URL + data.getSeller.logo}
                            alt={data.getSeller.shopName}
                            width={300}
                            height={300}
                            className="w-[110px] h-[110px] rounded-full border-2 border-solid border-main" />
                        <div>
