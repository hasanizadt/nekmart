...
const OrderForm = () => {
    ...
    const [{ error, fetching }, mutate] = useMutation(ADD_PRE_ORDER);
        ...
        <section>
            <Container className="py-6">
                ...
                <div className="flex gap-2 items-center">
                    {data?.getSiteSettings.logo ? <Image
                        src={process.env.NEXT_PUBLIC_IMAGE_URL + data.getSiteSettings.logo}
                        alt="Nekmart"
                        width={540}
                        height={120}
                        className="w-[200px]" /> : <h4 className="text-2xl font-bold text-main">Nekmart</h4>}
                            ...
                            <div className="col-span-3">
                                {fields.map((_, i) => (
                                    <div key={i} className="flex gap-3 mb-7">
                                        ...
                                        <button
                                            className={`text-main text-sm ${fields.length <= 1 && "opacity-50"}`}
                                ...
                                <div className="mt-2">
                                    <button
                                        onClick={() => append({ url: "" })}
                                        className="border border-solid border-main text-main py-1.5 px-2 text-sm rounded-md font-medium"
                                        type="button">
                                        Add more
                                    </button>
                                </div>
