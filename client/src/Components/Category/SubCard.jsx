            ...
            <ul className="grid grid-cols-4 gap-6">
                {data.category?.map((category, ci) => (
                    <Fragment key={ci}>
                        <li>
                            <Link
                                href={`/category/${slugify(data.name, { lower: true })}-${data.id}?category=${slugify(category.name, { lower: true })}-${category.id}`}
                                className="p-5 border border-solid border-gray-300 rounded-md block text-center">
                                <span className="flex-[0_0_10%]">
                                    {category.image ? <Image
                                        ...
                                        width={100}
                                        height={100}
                                        className="w-[80%] mx-auto rounded-md" /> : <Image
                                        src="/placeholder-300.png"
                                        alt={category.name}
                                        width={100}
                                        height={100}
                        ...
                        {category?.sub_category.map((sub, si) => (
                            <li key={si}>
                                <Link
                                    href={`/category/${slugify(data.name, { lower: true })}-${data.id}?category=${slugify(category.name, { lower: true })}-${category.id}&subcategory=${slugify(sub.name, { lower: true })}-${sub.id}`}
                                    className="p-5 border border-solid border-gray-300 rounded-md block text-center">
                                    <span className="flex-[0_0_10%]">
                                        {sub.image ? <Image
