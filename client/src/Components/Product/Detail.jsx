...
const Detail = () => {
    ...
    //Urql
    const [{ data }] = useQuery({ query: GET_SINGLE_PRODUCT, variables: { getProductId: id } });
    const [{ data: review }] = useQuery({ query: GET_REVIEWS, variables: { productId: id } })

    //State
    const [count, setCount] = useState(Number(data?.getProduct.minPurchase));
    const [variant, setVariant] = useState([]);
    const [updateInfo, setUpdateInfo] = useState(
        { price: data?.getProduct.price, totalPrice: Number(data?.getProduct.totalPrice), quantity: data?.getProduct.quantity }
    );
    ...
    return (
        <div>
            ...
            <div className="my-2 flex gap-2">
                <p
                    className="opacity-50 border-r border-solid border-gray-300 pr-2 text-[15px]">{review?.getReviewByProduct.length.toString().padStart(2, "0")} reviews</p>
            ...
            <div>
                <p className="text-sm mb-1.5">
                    ...
                    <span
                        className={`${Number(data.getProduct.quantity) > 0 ? "text-green-600" : "text-main"} font-semibold`}>{Number(data.getProduct.quantity) > 0 ? "In Stock" : "Out Of Stock"}</span>
                </p>
                        ...
                        <div key={i} className="my-3">
                            ...
                            <div className="flex gap-2 mt-2">
                                {item.selected.map((v, vi) => (
                                    <button
                                        key={vi}
                                        className={`border border-solid border-main px-2.5 py-0.5 rounded ${variant.find(s => s.variant === v) ? "bg-main text-white" : ""}`}
                                        onClick={() => onVariantSelect(
                                            { id: item.id, name: data.getProduct.attributes?.attributeIds.find(a => a.id === item.id)?.name, variant: v }
                                        )}>
                                        {v}
                                    </button>
                                ))}
