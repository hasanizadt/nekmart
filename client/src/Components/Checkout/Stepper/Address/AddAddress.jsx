...
import { Dialog, Input, Radio, Textarea, Select, Option } from "@material-tailwind/react";
...
import { ADD_ADDRESS, GET_ADDRESS } from "@/Urql/Query/Checkout/checkout.query";
...
const AddAddress = ({
    ...
    //Urql
    const [{ data, error, fetching }, mutate] = useMutation(ADD_ADDRESS);
    ...
    const onSubmit = (value) => {
        ...
        mutate({ addressInput: formData }).then(({ data }) => {
    ...
    return (
        <Fragment>
            <Notification
                ...
                {error?.message ?? data?.addAddress.message}
            </Notification>
            <Dialog
                open={open}
                handler={onClose}
                ...
                <div className="flex gap-4">
                    <h4 className="text-lg font-semibold flex-1">New Address</h4>
                    <button
                        className="bg-main bg-opacity-30 text-main rounded-md p-1"
                        onClick={onClose}>
                ...
                <hr className="mt-2" />
                <form
                    className="max-h-[500px] py-4 pr-4 overflow-auto"
                    ...
                    <div className="grid grid-cols-4 gap-2 items-start mb-7">
                        <label htmlFor="type" className="col-span-1 w-max">Gender</label>
                        <div className="col-span-3">
                    ...
                    <div className="mt-6 text-right">
                        <button
                            className="bg-main px-4 uppercase font-semibold py-2 text-white rounded-md text-sm relative"
                            type="submit"
                            disabled={fetching}>
                            Save
