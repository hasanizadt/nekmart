...
const AddressMenu = ({
    ...
    //Urql
    const [{ data: mark, error: markError, fetching: markFetching }, markMutate] = useMutation(MARK_AS_DEFAULT);
    const [{ data, error, fetching }, mutate] = useMutation(DELETE_ADDRESS);
    ...
    return (
        <div className="absolute top-4 right-4 z-40" ref={ref}>
            ...
            <div
                className={`absolute right-0 top-full bg-white w-[200px] shadow-3xl border border-solid border-gray-300 p-4 transition-all ${menu ? "visible translate-y-0 opacity-100" : "opacity-0 invisible translate-y-2"}`}>
                <ul>
                    <li
                        className="bg-white px-2 py-1 rounded-md hover:bg-main cursor-pointer select-none hover:text-white"
                        onClick={() => setEdit(true)}>
                        Edit
                    </li>
                    <li
                        className={`bg-white px-2 py-1 rounded-md hover:bg-main cursor-pointer select-none hover:text-white group relative ${item.default ? "opacity-50 pointer-events-none" : ""}`}
                        onClick={onMark}>
                        Set as Default
                        <div className="absolute right-2 top-1/2 -translate-y-1/2">
                            {markFetching &&
                                <div
                                    className="w-3 h-3 border-b-2 border-main group-hover:border-white rounded-full animate-spin ml-auto"></div>
                            }
                        </div>
                    </li>
                    <li
                        className="bg-white px-2 py-1 rounded-md hover:bg-main cursor-pointer select-none hover:text-white group relative"
                        onClick={onDelete}>
                        Delete
                        <div className="absolute right-2 top-1/2 -translate-y-1/2">
                            {fetching &&
            ...
            <UpdateAddress open={edit} onClose={() => setEdit(false)} item={item} />
        </div>
    );
};
