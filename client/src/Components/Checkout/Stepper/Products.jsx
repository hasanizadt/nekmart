...
const Products = () => {
    ...
    const [{ data: coupon, error, fetching: couponFetching }, mutate] = useMutation(APPLY_COUPON);
    ...
    const onSubmit = (value) => {
        mutate(
            { applyInput: { code: value.code, minPurchase: (Number(carts?.total) + Number(totalTax)).toString() } }
        ).then(({ data }) => {
    ...
    if (fetching) return (
        <div
            className="w-6 h-6 border-b-2 border-main rounded-full animate-spin mx-auto"></div>
    );
    ...
    return (
        <div>
                            ...
                            <thead>
                                <tr>
                                    {TABLE_HEAD.map((head, i) => (
                                        <th
                                            key={i}
                                            className={`border border-black border-opacity-5 bg-white px-3 py-1 ${head === "Action" ? "text-center" : "text-left"}`}>
                                            {i === 0 ? (<Checkbox
                                                color="red"
                ...
                <div className="col-span-4">
                    ...
                    <form className="flex mt-7" onSubmit={handleSubmit(onSubmit)}>
                        <input
                            ...
                            required />
                        <button
                            className="px-4 bg-main text-white font-medium rounded-e-md relative"
                            type="submit">
            ...
            <div className="flex gap-10 mt-20 items-center">
                ...
                <div>
                    <button
                        className={`bg-main py-2 px-5 rounded-md text-white font-semibold text-sm ${selected.length === 0 ? "opacity-40" : ""}`}
