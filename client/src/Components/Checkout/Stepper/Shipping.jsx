...
const Shipping = () => {
    ...
    //Urql
    const [{ data }] = useQuery({ query: GET_ADDRESS });
    const [{ data: shipping }] = useQuery({ query: GET_SHIPPING });
    ...
    useEffect(() => {
        if (data && data?.getAddress.length > 0) {
            setAddress?.(
                { shippingAddress: data?.getAddress[0].id, billingAddress: data?.getAddress[0].id }
            )
        }
    ...
    useEffect(() => {
        if (carts?.total) {
            getShippingFees(data?.getAddress[0].id)
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
        ...
        <div>
            <div className="grid grid-cols-12 gap-10 items-start">
                <div className="col-span-8 border border-solid border-gray-300 p-5">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-5">
                        {data?.getAddress.map((item, i) => (
                            <div
                                className={`flex gap-3 items-start relative border border-solid p-4 rounded-md ${item.id === address?.shippingAddress ? "border-main" : "border-gray-400"}`}
                                key={i}>
                                <div
                                    className={`border border-solid border-gray-400 w-4 h-4 rounded-full mt-[2px] cursor-pointer relative after:w-3 after:h-3 after:rounded-full after:left-1/2 after:top-1/2 after:absolute after:-translate-x-1/2 after:-translate-y-1/2 ${item.id === address?.shippingAddress ? "after:bg-main" : "after:bg-transparent"}`}
                                    onClick={() => onShippingChange(item.id)}></div>
                                <div>
                                ...
                                {item.default &&
                                    <p
                                        className="absolute bottom-3 right-3 text-xs bg-main px-1 py-px rounded text-white">Default</p>
                                }
                            </div>
                        ))}
                    </div>
                    ...
                    {!check &&
                        <div className="grid grid-cols-2 gap-4 mt-5">
                            {data?.getAddress.map((item, i) => (
                                <div
                                    className={`flex gap-3 items-start relative border border-solid p-4 rounded-md ${item.id === address?.billingAddress ? "border-main" : "border-gray-400"}`}
                                    key={i}>
                                    <div
                                        className={`border border-solid border-gray-400 w-4 h-4 rounded-full mt-[2px] cursor-pointer relative after:w-3 after:h-3 after:rounded-full after:left-1/2 after:top-1/2 after:absolute after:-translate-x-1/2 after:-translate-y-1/2 ${item.id === address?.billingAddress ? "after:bg-main" : "after:bg-transparent"}`}
                                        onClick={() => onBillingChange(item.id)}></div>
                                    <div>
                                        <p className="text-sm font-medium">
                                            <span className="opacity-60">Name: </span>{item.name}
                                        </p>
                    ...
                    <div
                        className="border border-solid border-gray-200 mt-10 text-center py-4 bg-[#f6f6f6] rounded-md cursor-pointer select-none"
                        onClick={() => setAdd(true)}>
                        <p className="text-3xl opacity-80">+</p>
                        <p className="opacity-80">Add Address</p>
                    </div>
                    <AddAddress open={add} onClose={() => setAdd(false)} />
                </div>
                <div className="col-span-4">
                    ...
                    <div className="mt-5">
                        <button
                            className={`bg-main py-2 text-center w-full rounded-md text-sm font-medium text-white ${!address?.shippingAddress ? "opacity-50" : ""}`}
                            disabled={!address?.shippingAddress}
                            onClick={handleNext}>
                            Proceed to payment
                        </button>
                    </div>
            ...
            <div className="mt-4">
                <button
                    onClick={handlePrev}
                    className="flex gap-1.5 items-center text-base text-main font-medium">
