...
const ProductCard = ({
    ...
    return (
        <div className="bg-white shadow-5xl py-4 px-5 text-center rounded-lg relative">
            ...
            {showDiscount &&
                <div
                    className="bg-white absolute -left-[6%] top-[6%] shadow-5xl flex gap-1 px-3 py-[5px] rounded">
                    <span className="text-sm font-semibold text-main">OFF</span>
                    <span
                        className="bg-main text-sm font-semibold py-px px-1 rounded-[3px] text-white">{product.discount}{product.discountUnit === "percent" ? "%" : "৳"}</span>
                </div>
            }
        </div>
    );
