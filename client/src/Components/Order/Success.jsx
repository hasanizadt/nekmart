...
const Success = () => {
    ...
    const [{ data, error }] = useQuery(
        { query: GET_ORDER, variables: { trackInput: { trackId: `NEK-${router.query.id}` } } }
    );

    if (error) return (<p className="text-center mt-5 text-main font-medium">{error?.message}</p>);

    if (!data) return null;

    return (
        <div
            className="w-[60%] mx-auto border border-solid border-gray-300 rounded-md py-6 px-6">
            <div className="text-center">
                ...
                <Link
                    href={`/dashboard/purchase-history/${data?.trackOrder.id}`}
                    className="bg-main text-white rounded-md py-2.5 px-5 text-sm uppercase font-medium">
                    See Details
                </Link>
            </div>
            <div className="mt-8">
