...
const Lists = () => {
    //Urql
    const [{ data }] = useQuery({ query: GET_MY_ORDER });
            ...
            <div>
                {data.getOrderByUser.map((item, i) => (
                    <div className="border border-solid border-gray-300 mb-5 rounded" key={i}>
                        <div
                            className="py-3 px-4 flex gap-5 border-b border-solid border-gray-300 items-center">
                            ...
                            <Link
                                href={`/dashboard/purchase-history/${item.id}`}
                                className="text-main uppercase text-sm font-medium">
                                Manage
                            </Link>
                        </div>
                                ...
                                <Fragment key={si}>
                                    {seller.products.map((product, pi) => (
                                        <div key={pi} className="flex gap-7 mb-5 items-center">
                                            <Image
                                                src={process.env.NEXT_PUBLIC_IMAGE_URL + product.productId?.images[0]}
