...
const ReviewChecker = ({
    ...
    //Urql
    const [{ data: check }, refetchCheck] = useQuery({ query: CHECK_REVIEW, variables: { checkReviewInput: { product: id } } });
    const [{ data, error, fetching }, mutate] = useMutation(ADD_REVIEWS);
    const [_, refetch] = useQuery({ query: GET_REVIEWS, variables: { productId: id } })
    ...
    return (
        <div>
            ...
            {check?.reviewAvailability.success === false && status === "Delivered" &&
                <button
                    className="bg-main py-1 px-2 text-sm rounded-md text-white font-medium"
            ...
            <Dialog
                ...
                <form
                    ...
                    className="max-h-[450px] pr-4 overflow-auto">
                    ...
                    <div className="text-right mt-8">
                        <button
                            className="bg-main uppercase font-semibold py-2 text-white px-4 rounded-md text-[13px] relative"
                            type="submit"
                            disabled={fetching}>
