...
const Wishlist = () => {
    ...
    //Urql
    const [{ data }, refetch] = useQuery({ query: GET_WISHLIST });
    const [{ data: deleteData, error }, mutate] = useMutation(DELETE_WISHLIST);
            ...
            {data?.getWishlist.map((item, i) => (
                <div className="border border-solid border-gray-300 rounded-md" key={i}>
                    ...
                    <div className="p-4 flex gap-4">
                        ...
                        <Link
                            href={`/product/${slugify(item.product.name, { lower: true })}-${item.product.id}`}
                            className="flex gap-2 w-full bg-main items-center justify-center py-2 text-white rounded-md">
