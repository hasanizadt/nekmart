...
const Redeem = ({
    ...
    //Urql
    const [_, refetch] = useQuery({ query: GET_USER_POINTS });
    const [__, refetchCoupon] = useQuery({ query: GET_COUPON_CODE });
    const [{ data, error, fetching }, mutate] = useMutation(REDEEM_CODE);
        ...
        <Fragment>
            <tr>
                ...
                <td className={`${classes} text-right`}>
                    <button
                        className={`text-[13px] font-semibold uppercase py-1 px-3 bg-main text-white rounded relative ${(userPoints || 0) < item.points ? "opacity-50" : ""}`}
                        disabled={(userPoints || 0) < item.points}
                        onClick={onRedeem}>
                        <span className={fetching ? "opacity-50" : ""}>Redeem</span>
                        <div className="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2">
                            {fetching &&
                                <div
                                    className="w-4 h-4 border-b-2 border-white rounded-full animate-spin ml-auto"></div>
                            }
                        </div>
                    </button>
                </td>
