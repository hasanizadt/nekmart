...
const RefundList = () => {
    ...
    //Urql
    const [{ data }, refetch] = useQuery({ query: GET_REFUND_PRODUCT });
                                ...
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <div className="flex gap-2 items-center">
                                            <Image
                                                src={process.env.NEXT_PUBLIC_IMAGE_URL + item.refundableId?.productId?.images[0]}
                                                alt={item.refundableId?.productId?.name}
                                                width={70}
                                                height={70}
                                                className="w-[40px] rounded" />
                                            <Typography
                                                variant="small"
                                                className="font-semibold text-[14px] line-clamp-1 max-w-[300px]">
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.status === "Pending" ? "text-yellow-600" : item.status === "Approved" ? "text-green-600" : "text-red-600"}`}>
                                            {item.status}
                                        </Typography>
                                    </td>
