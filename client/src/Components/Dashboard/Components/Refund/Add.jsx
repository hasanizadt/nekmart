...
const Add = ({
    ...
    //Urql
    const [{ data, error, fetching }, mutate] = useMutation(ADD_REFUND);
    ...
    return (
        <Fragment>
            <Notification open={notification1} handleClose={onNotification} severity="success">
            ...
            <Dialog
                ...
                size="md">
                <Notification open={notification2} handleClose={onNotification} severity="error">
                    {error?.message}
                </Notification>
                <DialogBody className="text-black p-5 max-h-[450px] overflow-auto">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <button
                            className="bg-main uppercase font-semibold py-2 text-white px-4 rounded-md text-sm relative w-max mt-4"
                            type="submit"
                            disabled={fetching}>
