            ...
            <div className="bg-main text-white text-center py-8 px-2">
                <div>
                    {data?.getProfile?.avatar ? (
                        <Image
                            src={isUrl(data.getProfile.avatar) ? data.getProfile.avatar : (process.env.NEXT_PUBLIC_IMAGE_URL + data.getProfile.avatar)}
                            alt={data.getProfile?.name || "Profile"}
                            width={60}
                            height={60}
            ...
            <ul className="my-4 px-3">
                {Navs.map((item, i) => (
                    <li key={i} className="mb-1">
                        <Link
                            href={item.url}
                            className={`flex gap-2 items-center py-2 px-2.5 rounded-md bg-opacity-30 ${item.id === active ? "bg-main" : "bg-transparent"}`}>
