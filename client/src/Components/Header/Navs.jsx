...
const Navs = () => {
    ...
    //Urql
    const [{ data }] = useQuery(
        { query: GET_RUNNING_FLASH, variables: { searchInput: { page: 1, limit: 50 } } }
    );
    ...
    return (
        <div className="flex gap-4">
            <div className="flex-[0_0_10%]">
                ...
                <Categories open={open} onClose={() => setOpen(false)} />
            </div>
                        ...
                        <div className="flex gap-6">
                            {data.getRunningFlashes.results.map((item, i) => (
                                <div key={i} className="whitespace-nowrap">
                                    <Link
                                        href={`/flash-deals/${slugify(item.title, { lower: true })}-${item.id}`}
                                        className="text-[15px]">{item.title}</Link>
                                </div>
                            ))}
                        </div>
                    </div>
