...
const Categories = ({
    ...
    //Urql
    const [{ data }] = useQuery(
        { query: GET_CATEGORIES, variables: { searchInput: { page: 1, limit: 200 } } }
    );
                ...
                {data?.getMainCategories.results.map((item, Ig) => (
                    <div key={Ig} className="my-1">
                        <div className="flex gap-3 items-center">
                            ...
                            <button
                                className={`flex-1 w-full text-right ${item.category.length === 0 ? "opacity-30" : ""}`}
                                onClick={() => categoryHandler(item.id)}
                                disabled={item.category.length === 0}>
                                <Icon
                                    className={`ml-auto text-[28px] transition-all ${category === item.id ? "rotate-90" : "rotate-0"}`}
                                ...
                                {item.category.map((cates, Ic) => (
                                    <div key={Ic} className="my-1">
                                        <div className="flex gap-3 items-center">
                                            ...
                                            <button
                                                className={`flex-1 w-full text-right ${cates.sub_category.length === 0 ? "opacity-30" : ""}`}
                                                onClick={() => subHandler(cates.id)}
                                                disabled={cates.sub_category.length === 0}>
                                                <Icon
                                                    className={`ml-auto text-[28px] transition-all ${sub === cates.id ? "rotate-90" : "rotate-0"}`}
                                                    icon="ei:chevron-right" />
                                            </button>
                                        </div>
