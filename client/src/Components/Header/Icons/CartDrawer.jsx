...
const CartDrawer = ({
    ...
    //Urql
    const [{ data }, refetch] = useQuery({ query: GET_CART });
    const [{ data: deleteData, error }, mutate] = useMutation(DELETE_CART);
    const [_, increaseMutate] = useMutation(INCREASE_CART);
    const [__, decreaseMutate] = useMutation(DECREASE_CART);
            ...
            <Drawer
                ...
                <div className="grid grid-cols-2 items-center">
                    ...
                    <div className="text-right">
                        <button
                            className="bg-main p-1.5 bg-opacity-10 rounded text-main"
                            onClick={onClose}>
                ...
                <div>
                    {data.getCarts.map((item, i) => (
                        <div
                            className={`flex gap-4 my-2 items-center ${data.getCarts.length - 1 !== i ? "border-b border-solid border-gray-300 mb-4 pb-4" : ""}`}
                            key={i}>
                            <div className="text-center bg-main text-white px-2 py-2 rounded-3xl">
                                <span
                                    className={`mb-1 block cursor-pointer select-none ${item.reserved >= Number(getCalculatedPrice(item)?.quantity) ? "opacity-50 pointer-events-none" : ""}`}
                                    onClick={() => increaseHandler(item.id)}>
                                    <Icon icon="akar-icons:plus" />
                                </span>
                                <span>{item.reserved}</span>
                                <span
                                    className={`mt-0.5 block cursor-pointer select-none ${Number(item.productId.minPurchase) >= item.reserved ? "opacity-50 pointer-events-none" : ""}`}
                            ...
                            <div className="flex-1 flex gap-3">
                                <div className="flex-[0_0_17%]">
                                    <Image
                                        src={process.env.NEXT_PUBLIC_IMAGE_URL + getCalculatedPrice(item).image}
                                ...
                                <div>
                                    <Link
                                        href={`/product/${slugify(item.productId.name, { lower: true })}-${item.id}`}
                                        className="text-main line-clamp-1 text-[15px] font-medium mb-1">
                                        {item.productId.name}
                                    </Link>
                            ...
                            <div className="text-right">
                                <button
                                    className="border border-solid border-gray-500 p-2 rounded-md hover:bg-main hover:text-white hover:border-main"
                ...
                <div className="absolute bottom-0 left-0 p-2 w-full">
                    <Link
                        href="/checkout"
                        className="bg-main w-full block py-2 text-center rounded-md text-white font-medium">
                        Checkout (৳{data.getCarts.reduce(
                        (a, b) => +a + +((getCalculatedPrice(b)?.price || 0) * Number(b.reserved)),
                        0
                    )})
                    </Link>
                </div>
