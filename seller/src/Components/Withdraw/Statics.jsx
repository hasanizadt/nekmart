...
const Statics = () => {
    ...
    //Urql
    const [{ data: sellerProfile }] = useQuery({ query: GET_SELLER_PROFILE })
    const [{ data }, refetch] = useQuery(
        { query: GET_INCOME_STATISTIC, variables: { sellerId: sellerProfile?.getSellerProfile.id } }
    );
        ...
        <div className="border border-solid border-gray-300 rounded">
            ...
            <div className="p-5">
                ...
                <div className="mt-8 flex gap-5">
                    ...
                    <Link
                        href="/money-withdraw/previous-withdraw"
                        className="bg-main py-2 px-3 text-sm text-white rounded-md w-max mx-auto">
                        Previous Withdraw
                    </Link>
                </div>
            </div>
            <AddBankInformation open={open} onClose={() => setOpen(false)} />
        </div>
    );
};
