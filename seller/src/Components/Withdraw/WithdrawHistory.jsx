...
const WithdrawHistory = () => {
    ...
    //Urql
    const [{ data }] = useQuery(
        { query: GET_PREVIOUS_WITHDRAW, variables: { searchInput: { page: 1, limit: 20 } }, requestPolicy: "cache-and-network" }
    );
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.status === "Processing" ? "text-yellow-600" : "text-green-600"}`}>
                                            {item.status}
                                        </Typography>
                                    </td>
                                    <td className={classes}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getWithdrawalBySeller.meta.totalPages && data.getWithdrawalBySeller.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getWithdrawalBySeller?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getWithdrawalBySeller?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
