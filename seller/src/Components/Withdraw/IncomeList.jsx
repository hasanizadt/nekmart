...
const IncomeList = () => {
    ...
    //Urql
    const [{ data }] = useQuery(
        { query: GET_INCOME_HISTORY, variables: { searchInput: { page: 1, limit: 20 } }, requestPolicy: "cache-and-network" }
    );
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.paySuccess ? "text-green-600" : "text-yellow-600"}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getIncomeHistory.meta.totalPages && data.getIncomeHistory.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getIncomeHistory?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getIncomeHistory?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
