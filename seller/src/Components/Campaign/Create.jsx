...
const Create = () => {
    ...
    //Urql
    const [{ data: profile }] = useQuery({ query: GET_SELLER_PROFILE });

    const [{ data: flashData, fetching: flashFetching }] = useQuery(
        { query: GET_RUNNING_FLASH, variables: { searchInput: { page: 1, limit: 20, search: productSearch } } }
    );

    const [{ data: productData, fetching: productFetching }, refetch] = useQuery(
        { query: GET_NO_FLASH_PRODUCT, variables: { searchInput: { page: 1, limit: 20, search: flashSearch }, sellerId: profile?.getSellerProfile.id } }
    );

    const [{ data, error, fetching }, mutate] = useMutation(UPDATE_FLASH_PRODUCT);
            ...
            <div className="border border-solid border-gray-200 rounded-md">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-5">
                    ...
                    <div className="text-right mt-8">
                        <button
                            className="bg-main uppercase font-semibold py-3 text-white px-8 rounded-md text-sm relative"
                            type="submit"
                            disabled={fetching}>
