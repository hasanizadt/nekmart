    ...
    const onImageChange = async (imageList) => {
        ...
        if (imageList.length > 0) {
            ...
            try {
                ...
                const res = await s3.uploadFile(imageList[0].file);
                onChange(res.key)
                if (value) {
                    ...
                    {({
                        ...
                        <div>
                            <button
                                ...
                                type="button">
                                <p
                                    className={` px-3 py-2 opacity-80 ${error ? "bg-red-50 text-red-500" : "bg-white_hover"}`}>Browser</p>
                                {isDragging &&
                            ...
                            <div className="flex gap-2 mt-2">
                                {imageList.map((image, i) => (
                                    <div
                                        key={i}
                                        className="border border-solid border-gray-200 rounded relative w-[150px]">
                                        <button
                                            onClick={() => onImageRemove(i)}
                                            className="absolute -top-2 -right-2 w-[25px] h-[25px] bg-white_hover text-main rounded-full flex justify-center items-center"
                                            type="button">
                                        ...
                                        <Image
                                            ...
                                            width={width}
                                            height={height}
                                            className="w-[150px] h-[60px] object-cover object-top rounded-t" />
                                        ...
                                        <div>
                                            <div className="w-full relative h-[2px] bg-main bg-opacity-40">
                                                <div
                                                    className="absolute bg-green-600 left-0 top-0 bottom-0 w-0 transition-[width] ease-out duration-700"
                                ...
                                {value && imageList.length === 0 &&
                                    <div
                                        className="border border-solid border-gray-200 rounded relative w-[100px]"
                                        key={value}>
                                        <Image
                                            ...
                                            width={width}
                                            height={height}
                                            className="w-[100px] object-cover object-top rounded"
                                            key={value}
                                            priority />
                                    </div>
                                }
