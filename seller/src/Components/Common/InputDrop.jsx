...
const InputDrop = ({
    ...
    //State
    const [value, setValue] = useState(match ? match[1] : "");
    const [open, setOpen] = useState();
    const [selected, setSelected] = useState(match ? match[2] : "year");
    ...
    return (
        <div className="relative">
            ...
            <div className="absolute right-0 top-0 select-none cursor-pointer" ref={ref}>
                <div
                    className="flex items-center justify-center gap-1.5 select-none h-10 px-2"
                    onClick={() => setOpen(!open)}>
                    <div className="flex gap-1.5">
                        <span className="capitalize text-sm">{selected}</span>
                        <div
                            className={`w-5 h-5 text-blue-gray-400 transition-all  mt-px ${open ? "rotate-180" : "rotate-0"}`}><svg
