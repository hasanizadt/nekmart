...
const ProductInformation = () => {
    ...
    //Urql
    const [{ data: mainCategory, fetching: mainCategoryFetching }] = useQuery(
        { query: GET_MAIN_CATEGORY, variables: { searchInput: { page: 1, limit: 20, search: mainSearch } } }
    );

    const [{ data: category, fetching: categoryFetching }] = useQuery(
        { query: GET_CATEGORY, variables: { searchInput: { page: 1, limit: 20, search: categorySearch } } }
    );

    const [{ data: subData, fetching: subFetching }] = useQuery(
        { query: GET_SUB_CATEGORY, variables: { searchInput: { page: 1, limit: 20, search: subSearch } } }
    );

    const [{ data: brand, fetching: brandFetching }] = useQuery(
        { query: GET_BRAND, variables: { searchInput: { page: 1, limit: 20, search: brandSearch } } }
    );

    const [{ data: tagData, fetching: tagFetching }] = useQuery(
        { query: GET_TAGS, variables: { searchInput: { page: 1, limit: 20, search: tagSearch } } }
    );
                                ...
                                <CustomSelect
                                    ...
                                    search={
                                        <div className="mb-2 relative">
                                            ...
                                            <Icon
                                                icon="gala:search"
                                                className="absolute left-2 top-1/2 -translate-y-1/2 text-lg text-blue-gray-900" />
                                        </div>
                                    }
                ...
                <div className="grid grid-cols-4 gap-2 items-start mb-7">
                    ...
                    <div className="col-span-3">
                        <Controller
                            ...
                            name="warranty"
                            render={({ field: { onChange, value } }) => (
                                <InputDrop options={["year", "month", "day"]} onChange={onChange} value={value} />
                            )} />
                    </div>
                </div>
                <div className="grid grid-cols-4 gap-2 items-start mb-7">
