...
const ProductVariation = () => {
    ...
    const [{ data, fetching }] = useQuery(
        { query: GET_ATTRIBUTES, variables: { searchInput: { search: search, limit: 20, page: 1 } } }
    );
    ...
    const generateVariant = () => {
        if (attributes && attributes?.attributeIds?.length > 0) {
            ...
            setValue?.("attributes.selectedVariant", variant);
        } else {
            setValue?.("attributes", {})
        }
    }
                                    ...
                                    <Controller
                                        ...
                                        render={({ field: { onChange, value } }) => (
                                            <MultiSelect
                                                ...
                                                label={data?.getAttributes.results.find(a => a.id === item.id)?.name}
                                                fetching={fetching}
