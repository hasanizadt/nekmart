...
const Lists = () => {
    ...
    const [{ data }, refetch] = useQuery(
        { query: GET_OWN_PRODUCTS, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
    const [{ data: deleteData, error, fetching }, mutate] = useMutation(DELETE_PRODUCT);
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.is_approved ? "text-green-600" : "text-red-600"}`}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.visibility ? "text-green-600" : "text-red-600"}`}>
                                        ...
                                        <div className="flex gap-3 justify-center items-center">
                                            <div>
                                                <Tooltip
                                                    ...
                                                    <button>
                                                        <Link
                                                            href={`/product/update-product/${slugify(item.name, { lower: true })}-${item.id}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getOwnSellerProducts.meta.totalPages && data.getOwnSellerProducts.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getOwnSellerProducts?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getOwnSellerProducts?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
