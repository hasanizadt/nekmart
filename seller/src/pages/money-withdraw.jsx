...
export const getServerSideProps = async (ctx) => {
    ...
    //--//
    const { data: sellerProfile } = await client.query(GET_SELLER_PROFILE, {}, {
        fetchOptions,
        requestPolicy: "network-only"
    }).toPromise();

    //--//
    await client.query(GET_INCOME_STATISTIC, { sellerId: sellerProfile?.getSellerProfile.id }, {
        fetchOptions,
        requestPolicy: "network-only"
    }).toPromise();

    //--//
    await client.query(GET_INCOME_HISTORY, { searchInput: { page: 1, limit: 20 } }, {
        fetchOptions,
        requestPolicy: "network-only"
    }).toPromise();

    //--//
    await client.query(GET_PROCESSING_WITHDRAW, {}, {
        fetchOptions,
        requestPolicy: "network-only"
    }).toPromise();
