...
const Lists = () => {
    ...
    const [{ data }, refetch] = useQuery(
        { query: GET_PRE_ORDER, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
    const [{ data: deleteData, error, fetching }, mutate] = useMutation(DELETE_PRE_ORDER);
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        {item.productImage &&
                                            <Image
                                                src={process.env.NEXT_PUBLIC_IMAGE_URL + item.productImage[0]}
                                                alt={item.firstName}
                                                width={100}
                                                height={100}
                                    ...
                                    <td className={`${classes}`}>
                                        <div className="flex gap-3 justify-center items-center">
                                            <div>
                                                ...
                                                <Details open={detail === item.id} onClose={() => setDetail(null)} data={item} />
                                            </div>
                                            <div>
                                                <Tooltip
                                                    content="Delete Brand"
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getPreorder.meta.totalPages && data.getPreorder.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getPreorder?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getPreorder?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
