...
const Lists = () => {
    ...
    const [{ data }, refetch] = useQuery(
        { query: GET_ATTRIBUTES, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
    const [{ data: deleteData, error, fetching }, mutate] = useMutation(DELETE_ATTRIBUTE);
                                        ...
                                        <div className="flex gap-3 justify-center items-center">
                                            <div>
                                                <Tooltip
                                                    ...
                                                    <button>
                                                        <Link
                                                            href={`/attributes/update-attribute/${slugify(item.name, { lower: true })}-${item.id}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getAttributes.meta.totalPages && data.getAttributes.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getAttributes?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getAttributes?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
