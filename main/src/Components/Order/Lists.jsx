...
const Lists = () => {
    ...
    //Urql
    const [{ data }] = useQuery(
        { query: GET_ORDERS, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.payment?.paymentMethod === "cash" ? "text-green-600" : item.paymentStatus ? "text-green-600" : "text-red-600"}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getOrders.meta.totalPages && data.getOrders.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getOrders?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getOrders?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
