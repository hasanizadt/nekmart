...
const Lists = () => {
    ...
    const [{ data }, refetch] = useQuery(
        { query: GET_SHIPPINGS, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
    const [{ data: deleteData, error, fetching }, mutate] = useMutation(DELETE_SHIPPINGS);
    const [{ data: activeData, error: activeError, fetching: activeFetching }, activeMutate] = useMutation(CHANGE_ACTIVE_STATUS);
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.active ? "text-green-600" : "text-yellow-600"}`}>
                                        ...
                                        <div className="flex gap-3 justify-center items-center">
                                            <div>
                                                <Tooltip
                                                    ...
                                                    <button>
                                                        <Link
                                                            href={`/shippings/update-shipping/${slugify(item.name, { lower: true })}-${item.id}`}>
                                            ...
                                            <div>
                                                <Tooltip
                                                    ...
                                                    placement="top"
                                                    className="text-xs bg-black bg-opacity-70">
                                                    <Popover animate={animation} placement="bottom">
                                                        <PopoverHandler>
                                                            <button>
                                                        ...
                                                        <PopoverContent className="text-black py-2 px-2">
                                                            <button
                                                                className="bg-white py-2 px-2 rounded-md capitalize text-black font-semibold hover:bg-main hover:text-white"
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getShippings.meta.totalPages && data.getShippings.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getShippings?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getShippings?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
