...
const Lists = () => {
    ...
    //Urql
    const [{ data }] = useQuery(
        { query: GET_UNAPPROVED_PRODUCTS, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getUnapprovedProduct.meta.totalPages && data.getUnapprovedProduct.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getUnapprovedProduct?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getUnapprovedProduct?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
