...
const Edit = () => {
    ...
    const [{ data: mainCategory }] = useQuery(
        { query: GET_MAIN_CATEGORIES, variables: { searchInput: { limit: 20, page: 1, search: search } }, requestPolicy: "cache-and-network" }
    )
    const [{ data: categoryData }, refetch] = useQuery({ query: GET_SINGLE_CATEGORY, variables: { getCategoryId: id } })
    const [{ data, error, fetching }, mutate] = useMutation(UPDATE_CATEGORY);
