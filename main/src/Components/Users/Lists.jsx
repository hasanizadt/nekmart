...
const Lists = () => {
    ...
    const [{ data }, refetch] = useQuery(
        { query: GET_USERS, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
    const [{ data: banData, error, fetching }, mutate] = useMutation(BAN_OR_UNBANNED_USER);
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.is_banned ? "text-red-600" : "text-green-600"}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getUsers.meta.totalPages && data.getUsers.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getUsers?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getUsers?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
