...
const Lists = () => {
    ...
    const [{ data }, refetch] = useQuery(
        { query: GET_ADMINS, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
    const [{ data: deleteData, error, fetching }, mutate] = useMutation(DELETE_ADMIN);
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] capitalize ${item.role === "editor" ? "text-yellow-600" : item.role === "moderator" ? "text-purple-600" : "text-pink-600"}`}>
                                            {item.role}
                                        </Typography>
                                    </td>
                                    <td className={`${classes}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getAdmins.meta.totalPages && data.getAdmins.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getAdmins?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getAdmins?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
