...
const Lists = () => {
    ...
    const [{ data }, refetch] = useQuery(
        { query: GET_FLASHES, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
    const [{ data: deleteData, error, fetching }, mutate] = useMutation(DELETE_FLASH);
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${getStatus(item.start, item.expires) === "Upcoming" ? "text-yellow-600" : getStatus(item.start, item.expires) === "Running" ? "text-green-600" : "text-red-600"}`}>
                                        ...
                                        <div className="flex gap-3 justify-center items-center">
                                            <div>
                                                <Tooltip
                                                    ...
                                                    <button>
                                                        <Link
                                                            href={`/flash-sale/update-flash-sale/${slugify(item.title, { lower: true })}-${item.id}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getFlashes.meta.totalPages && data.getFlashes.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getFlashes?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getFlashes?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
