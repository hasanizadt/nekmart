...
const ReleasePayment = ({
    ...
    //Urql
    const [{ data, error, fetching }, mutate] = useMutation(RELEASE_PAYMENT);
    const [_, refetchWithdraw] = useQuery(
        { query: GET_PREVIOUS_WITHDRAW, variables: { sellerId: sellerId, searchInput: { page: 1, limit: 20 } } }
    )
    ...
    return (
        <Dialog
            ...
            <DialogBody className="text-black p-5 max-h-[450px] overflow-auto">
                ...
                <div
                    className="flex gap border border-solid border-gray-300 bg-gray-200 rounded-md mt-6">
                    <p
                        className="flex-[0_0_60%] border-r border-gray-300 py-2 text-base font-medium px-4">Due to seller</p>
                ...
                <div className="flex gap-3 justify-end mt-7 items-center">
                    ...
                    <button
                        className={`bg-main py-1 px-4 rounded text-white text-sm font-semibold relative ${currentIncome <= 0 ? "opacity-60" : ""} ${!lastPaymentDate || moment(lastPaymentDate).add(15, "days") > moment() ? "opacity-60" : ""}`}
                        disabled={currentIncome <= 0 || (!lastPaymentDate || moment(lastPaymentDate).add(15, "days") > moment())}
