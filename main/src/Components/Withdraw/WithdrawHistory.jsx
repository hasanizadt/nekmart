...
const WithdrawHistory = () => {
    ...
    //Urql
    const [{ data }] = useQuery(
        { query: GET_PREVIOUS_WITHDRAW, variables: { sellerId: id, searchInput: { page: 1, limit: 20 } }, requestPolicy: "cache-and-network" }
    );
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getWithdrawalByAdmin.meta.totalPages && data.getWithdrawalByAdmin.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getWithdrawalByAdmin?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getWithdrawalByAdmin?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
