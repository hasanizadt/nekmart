...
const Statics = () => {
    ...
    let name = parts.map(part => part.charAt(0).toUpperCase() + part.slice(1)).join(' ').replace(id, "");

    //Urql
    const [{ data }, refetch] = useQuery({ query: GET_INCOME_STATISTIC, variables: { sellerId: id } });
    const [{ data: platform }] = useQuery({ query: GET_PLATFORM });

    //Handler
    const getIncomes = () => {
        let upcomingIncome = data?.getIncomeStatics?.upcomingIncomes?.reduce((acc, item) => (item?.income || 0) + acc, 0) || 0;
        upcomingIncome = Math.round(
            upcomingIncome - (upcomingIncome * (Number(platform?.getPlatform.charge) / 100))
        );

        let currentIncome = data?.getIncomeStatics?.currentIncomes?.reduce((acc, item) => (item.income || 0) + acc, 0) || 0;
        currentIncome = Math.round(
            currentIncome - (currentIncome * (Number(platform?.getPlatform.charge) / 100))
        );
    ...
    return (
        <div className="border border-solid border-gray-300 rounded">
            <h4 className="px-3 py-3 border-b border-solid border-gray-300">{name}</h4>
            <div className="p-5">
                <div className="grid grid-cols-2 gap-4 w-[60%] mx-auto">
                    <div
                        className="bg-gradient-to-br from-[#eb4786] to-[#b854a6] rounded-md text-white">
                        <div className="p-5">
                    ...
                    <div
                        className="bg-gradient-to-br from-[#875fc0] to-[#5346ba] rounded-md text-white">
                        <div className="p-5">
                ...
                <div className="mt-8 text-center">
                    <button
                        className="bg-main py-2 px-3 text-sm text-white rounded-md w-max mx-auto"
                        onClick={() => setOpen(true)}>Release Payment</button>
                </div>
            </div>
            <ReleasePayment
                open={open}
                onClose={() => setOpen(false)}
                currentIncome={getIncomes().currentIncome}
                sellerId={id}
                incomeIds={data?.getIncomeStatics.currentIncomes?.map((item => item.id)) || []}
                lastPaymentDate={data?.getIncomeStatics.lastPaymentDate}
                refetch={refetch} />
        </div>
    );
};
