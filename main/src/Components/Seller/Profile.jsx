...
const Profile = () => {
    ...
    //Finding Ids
    const parts = router.query.id?.toString().split('-');
    const id = parts[parts.length - 1];

    //Urql
    const [{ data }, refetch] = useQuery({ query: GET_SINGLE_SELLER, variables: { getSellerByAdminId: id } });
    const [{ data: verifyData, error, fetching }, mutate] = useMutation(VERIFY_SELLER);
    ...
    return (
        <div>
            ...
            <div className="border border-solid border-gray-200 rounded-md">
                ...
                <div>
                    <Image
                        src={process.env.NEXT_PUBLIC_IMAGE_URL + data?.getSellerByAdmin.banner}
                        alt={data?.getSellerByAdmin.shopName}
                        width={1600}
                        height={300} />
                    <div className="flex items-end gap-3 -mt-14 px-5">
                        <div>
                            <Image
                                src={process.env.NEXT_PUBLIC_IMAGE_URL + data?.getSellerByAdmin.logo}
                                alt={data?.getSellerByAdmin.shopName}
                                width={300}
                                height={300}
                                className="border-2 border-solid border-main rounded-full w-[120px] h-[120px]" />
                        </div>
                        <div className="mb-2">
                ...
                <div className="p-5 grid grid-cols-2 gap-8">
                    <div>
                        ...
                        <p className="text-[15px] mb-2 flex items-center gap-1">
                            ...
                            <span
                                className={`font-semibold ${data?.getSellerByAdmin.is_verified ? "text-green-600" : "text-red-600"}`}>{data?.getSellerByAdmin?.is_verified ? "Verified" : "Non-verified"}</span>
                        </p>
                ...
                {!data?.getSellerByAdmin.is_verified &&
                    <div className="mt-2 p-5">
                        <button
                            className="bg-main uppercase font-semibold py-2 text-white px-4 rounded-md text-sm relative"
                            type="submit"
                            disabled={fetching}
                            onClick={onVerify}>
