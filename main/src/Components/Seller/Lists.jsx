...
const Lists = () => {
    ...
    const [unbanned, setUnbanned] = useState(null);
    ...
    const [{ data }, refetch] = useQuery(
        { query: GET_SELLERS, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
    const [{ data: banData, error, fetching }, mutate] = useMutation(BAN_OR_UNBANNED_SELLER);

    //On Delete Confirm
    const onBanConfirm = (id) => {
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.is_verified ? "text-green-600" : "text-red-600"}`}>
                                        ...
                                        <div className="flex gap-3 justify-center items-center">
                                            ...
                                            <div>
                                                <Tooltip
                                                    ...
                                                    <button>
                                                        <Link
                                                            href={`/seller/withdraw/${slugify(item.shopName, { lower: true })}-${item.id}`}>
                                            ...
                                            <div>
                                                <Tooltip
                                                    ...
                                                    <button>
                                                        <Link
                                                            href={`/seller/profile/${slugify(item.shopName, { lower: true })}-${item.id}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getSellersByAdmin.meta.totalPages && data.getSellersByAdmin.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getSellersByAdmin?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getSellersByAdmin?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
