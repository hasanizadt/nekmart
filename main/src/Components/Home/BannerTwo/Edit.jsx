...
import { UPDATE_BANNER_TWO, GET_BANNER_TWO } from "@/Urql/Query/Home/banner-two.query";
...
const Edit = ({
    ...
    //Urql
    const [_, refetch] = useQuery({ query: GET_BANNER_TWO });
    const [{ data, error, fetching }, mutate] = useMutation(UPDATE_BANNER_TWO);
    ...
    const onSubmit = (value) => {
        mutate({ bannerInput: value, updateBannerTwoId: banners.id }).then(({ data }) => {
            setNotification(true)
            if (data?.updateBannerTwo.success) {
    ...
    return (
        <div className="w-0">
            <Notification
                ...
                {error?.message ?? data?.updateBannerTwo.message}
            </Notification>
            <Dialog
                open={open}
                handler={onClose}
