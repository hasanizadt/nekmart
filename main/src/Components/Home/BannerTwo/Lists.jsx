...
const Lists = () => {
    ...
    //Urql
    const [{ data }, refetch] = useQuery({ query: GET_BANNER_TWO });
    const [{ data: deleteData, error, fetching }, mutate] = useMutation(DELETE_BANNER_TWO);
                ...
                {data?.getBannerTwo.map((item, i) => (
                    <div
                        ...
                        <div className="flex justify-center py-2 gap-3">
                            <button
                                className="bg-transparent p-1.5 rounded-md hover:bg-main hover:text-white"
                            ...
                            <Edit open={edit === item.id} onClose={() => setEdit(null)} banners={item} />
                        </div>
                    </div>
                ))}
