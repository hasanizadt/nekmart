...
const Lists = () => {
    ...
    const [{ data }, refetch] = useQuery(
        { query: GET_REFUND, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" }
    );
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <Typography
                                            variant="small"
                                            className={`font-semibold text-[14px] ${item.status === "Approved" ? "text-green-600" : item.status === "Pending" ? "text-yellow-600" : "text-main"}`}>
                                            {item.status}
                                        </Typography>
                                    </td>
                                    <td className={`${classes}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getRefundByAdmin.meta.totalPages && data.getRefundByAdmin.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getRefundByAdmin?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`}
                                disabled={pagination >= (data?.getRefundByAdmin?.meta?.totalPages || 0)}
                                onClick={() => setPagination(pagination + 1)}>
