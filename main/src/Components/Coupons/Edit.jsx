...
import { Datepicker } from "react-custom-datepicker-tailwind";
...
const Edit = () => {
    ...
    //Urql
    const [{ data: couponData }, refetch] = useQuery({ query: GET_SINGLE_COUPON, variables: { getSingleCouponByAdminId: id } })
    const [{ data, error, fetching }, mutate] = useMutation(UPDATE_COUPON);
                    ...
                    <div className="grid grid-cols-4 gap-2 items-start mb-7">
                        ...
                        <div className="col-span-3">
                            <Controller
                                ...
                                rules={{ required: true }}
                                render={({ field: { onChange, value } }) => (
                                    <Timepicker label="Expires time" value={value} onChange={onChange} />
                                )} />
                        </div>
                    </div>
